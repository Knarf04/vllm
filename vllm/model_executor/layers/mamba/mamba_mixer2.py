        ...
        if has_prefill:
            ...
            if "upi" in self.experiments or self.proper_upi:
                ...
                if self.proper_upi:
                    ...
                    # forget = 1 - torch.exp(self.A * dt_p)
                    # A = self.A * self.upi_interpolation_scale * forget
        ...
        if has_decode:
            ...
            if "upi" in self.experiments or self.proper_upi:
                ...
                if self.proper_upi:
                    ...
                    # forget = 1 - torch.exp(self.A.unsqueeze(-1) * dt_d)
                    # A_d = A_d * self.upi_interpolation_scale * forget
                    # hidden_states_scale = torch.expm1(self.upi_interpolation_scale * self.A.unsqueeze(-1) * forget * dt_d) / torch.expm1(self.A.unsqueeze(-1) * dt_d)
